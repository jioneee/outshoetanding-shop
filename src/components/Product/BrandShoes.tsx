...
const AdidasImage: ProductImg[] = [
  ...
  {
    id: 1,
    title: '슈퍼스타 80S DEUX', //슈퍼스타 80S DEUX
...
const ConverseImage: ProductImg[] = [
  ...
  { id: 1, title: '척 70 클래식 블랙 하이', image: 'https://www.converse.co.kr/web/product/big/202307/68a88e51c6a90d123a54d2ba05eb887f.jpg', price: 50000 },
];

const VansImage: ProductImg[] = [
  {
    id: 0,
    title: '볼트 og 올드스쿨 LX 블랙',
    image:
...
export const Nike = () => {
  ...
  const handleAddToCartClick = (id: number, title: string, img: string, price: number) => {
    setSelectedId(id);
      ...
      <ProductListWrap>
        {NikeImage.map((img) => (
          <div key={img.id}>
            ...
            <AddToCart type='button' onClick={() => handleAddToCartClick(img.id, img.title, img.image, img.price)}>
              add to cart
            </AddToCart>
          </div>
        ))}
        {isOpenModal && selectedImg !== null && <Modal onClick={clickCloseModal} id={selectedId} title={selectedTitle} img={selectedImg} price={selectedPrice} onAddToCart={() => {}} />}
      </ProductListWrap>
    </div>
  );
};
      ...
      <ProductListWrap>
        {AdidasImage.map((img) => (
          <div key={img.id}>
            ...
            <AddToCart onClick={() => handleAddToCartClick(img.id, img.title, img.image, img.price)}>add to cart</AddToCart>
          </div>
        ))}
        {isOpenModal && selectedImg !== null && <Modal onClick={clickCloseModal} id={selectedId} title={selectedTitle} img={selectedImg} price={selectedPrice} onAddToCart={() => {}} />}
      </ProductListWrap>
